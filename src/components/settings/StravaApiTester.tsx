...
import { testStravaApi, areCredentialsValid, canConnectToStrava, StravaApiTest } from '@/utils/tests/stravaApiTest';

/**
 * Strava API Tester Component
 * Provides a UI for testing the Strava API integration
 */
const StravaApiTester: React.FC = () => {
  const [loading, setLoading] = useState<boolean>(false);
  const [testResults, setTestResults] = useState<string | null>(null);
  const [testStatus, setTestStatus] = useState<'SUCCESS' | 'PARTIAL' | 'FAILED' | null>(null);

  /**
   * Run all Strava API tests
   */
  const runTests = async () => {
    ...
    try {
      ...
      if (!credsValid) {
        ...
        setTestResults('Strava credentials validation failed. Please check your Client ID and Client Secret configuration in the settings.');
        setLoading(false);
        return;
      }

      // Run the full test suite
      const results = await testStravaApi();
  ...
  const formatResults = (results: string): React.ReactNode => {
    ...
    return results.split('\n').map((line, index) => {
      ...
      } else if (line.startsWith('Token expires at:') || line.startsWith('New token would expire at:') || line.startsWith('Athlete:')) {
        return <div key={index} className="bg-slate-100 dark:bg-slate-800 p-2 rounded my-1 text-sm">{line}</div>;
      } else if (line.trim() === '') {
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-lg font-semibold">Strava API Test Utility</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="mb-4">
          <p className="text-sm text-muted-foreground mb-3">
            Test your Strava API integration to ensure it's properly configured and functioning. 
            This will validate credentials, test authentication, token refresh, and API connectivity.
          </p>
          <Button
            onClick={runTests}
            disabled={loading}
...
export default StravaApiTester;